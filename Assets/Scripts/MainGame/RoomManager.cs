...
namespace MainGame
{
    public class RoomManager : MonoBehaviourPunCallbacks
    {
        ...
        public readonly Dictionary<string, Color> ColorsDict = new Dictionary<string, Color>()
        {
            ...
            { "Grey", Color.grey },
            { "Brown", new Color(0.59f, 0.31f, 0.1f)},
        ...
        public string GetNextRoleName() => roles[nextPlayerRoleIndex];

        public string GetNextColor() => colors[nextPlayerRoleIndex];
        ...
        public void ClearTargets() // Clear targets list of local player
        {
            ...
            {
                case Seer seer:
                    seer.targets = new List<Role>();
                    break;
                case Werewolf werewolf:
                {
                    werewolf.targets = new List<Role>();
                    if (!VoteMenu.Instance.IsNight) werewolf.UpdateTransformation(false);
                    break;
                }
            }
        ...
        public int CheckIfEog() // Return 0 if not EOG | Return 1 if Werewolf won | Return 2 if Villager won | Return 3 if everyone is dead
        {
            ...
            foreach (Role role in players.Where(role => role.isAlive))
            {
                if (role is Werewolf) isThereWerewolf = true;
                else isThereVillager = true;
            }

            res = isThereWerewolf switch
            {
                true when !isThereVillager => 1,
                false when isThereVillager => 2,
                _ => res
            };
